...
# -*- coding: utf-8 -*-
"""
终极补丁 v5：零 emoji 版本，100% ASCII 安全
用法: python patch_v5.py
"""
...
'''
    ...
    print("OK: 插入 get_hot_topics_en()")
else:
    print("FAIL: 未找到插入位置（AI内容生成标记）")
...
new_call = '''    # 1. 抓热点(中英文分离)
    ...
    print(f"[Main] 中文话题: {len(topics_zh)} 个, 英文话题: {len(topics_en)} 个")'''

if old_call in content:
    content = content.replace(old_call, new_call)
    print("OK: 修改 main(): 分离中英文热点调用")
else:
    print("WARN: 未找到 main() 中的热点调用")

# 修改循环：中文用 topics_zh，英文用 topics_en
old_loop_zh = '    for i, topic in enumerate(topics):\n        slug_zh = topic_to_slug(topic)'
new_loop_zh = '    for i, topic in enumerate(topics_zh):\n        slug_zh = topic_to_slug(topic)'

if old_loop_zh in content:
    content = content.replace(old_loop_zh, new_loop_zh)
    print("OK: 修改 main(): 中文循环使用 topics_zh")
else:
    print("WARN: 未找到中文循环")

print("WARN: main() 中的英文循环需要手动调整（在中文循环后添加 topics_en 循环）")

# ========== 3. 内容偏向AI+金融：修改 prompt ==========
...
if old_prompt_zh in content:
    ...
    print("OK: 修改中文 prompt: 添加AI/金融偏向")
else:
    print("WARN: 未找到中文 prompt（可能已修改）")

# ========== 4. 脑图改渐变要点卡片（零emoji版本） ==========
old_mindmap = r'def _mindmap_html_block(markdown_text, slug, lang="zh"):'
new_mindmap = r'''def _mindmap_html_block(markdown_text, slug, lang="zh"):
    """将AI生成的要点转为渐变卡片HTML（零emoji版本）"""
...
<h3 style="margin:0 0 18px;font-size:1.1em;color:#333;display:flex;align-items:center;gap:8px">{section_title}</h3>
<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
'''
    
    # 编号图标（不用emoji，用圆圈数字 Unicode）
    num_icons = ["&#x2460;", "&#x2461;", "&#x2462;", "&#x2463;", "&#x2464;", "&#x2465;", "&#x2466;", "&#x2467;"]
    
    for i, point in enumerate(points):
        num_icon = num_icons[i % len(num_icons)]
        # 限制每点字数
...
<span style="font-size:1.2em;flex-shrink:0;margin-top:2px">{num_icon}</span>
...
# 使用正则替换（多行）
pattern = r'def _mindmap_html_block\(markdown_text, slug, lang="zh"\):.*?    return html'
match = re.search(pattern, content, re.DOTALL)
if match:
    content = content[:match.start()] + new_mindmap + content[match.end():]
    print("OK: 修改 _mindmap_html_block(): 渐变卡片（零emoji）")
else:
    print("WARN: 未找到 _mindmap_html_block()（可能已修改）")

# ========== 5. 添加 PropellerAds 广告位占位 ==========
propeller_code = r'''
...
if ad_bottom_pos > 0:
    ...
    if ad_end > 0:
        ...
        print("OK: 添加 PropellerAds 广告代码占位")
