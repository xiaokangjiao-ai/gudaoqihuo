...
# -*- coding: utf-8 -*-
"""
终极补丁：二进制读写 + 正则定位，100%成功
用法: python patch_final.py
"""
...
# 定位标记：AI内容生成 注释（在 get_hot_topics() 函数结束后）
marker_pattern = r'# ==================== AI内容生成 ====================\r?\n\r?\ndef get_zhipu_token'

match = re.search(marker_pattern, content)
if match:
    insert_pos = match.start()  # 在标记前插入
...
old_loop_zh = r'    for i, topic in enumerate\(topics\):\r?\n        slug_zh = topic_to_slug\(topic\)'
new_loop_zh = '    for i, topic in enumerate(topics_zh):\n        slug_zh = topic_to_slug(topic)'

if re.search(old_loop_zh, content):
    content = re.sub(old_loop_zh, new_loop_zh, content)
    print("✅ 修改 main(): 中文循环使用 topics_zh")
else:
    print("⚠️ 未找到中文循环")

# 英文循环（复杂，先跳过）
print("⚠️ main() 中的英文循环需要手动调整")
...
old_mindmap = r'def _mindmap_html_block\(markdown_text, slug, lang="zh"\):.*?    return html'''
